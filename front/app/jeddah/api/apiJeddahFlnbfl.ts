...
export async function ApiJeddahFlnbflTmplte() {
    try {
        const rspnse = await ApiGlobalAxiospParams.get(
            "/jeddah/flnbfl/tmplte", { responseType: "blob" });
...
export async function ApiJeddahFlnbflUpload(
...
export async function ApiJeddahflnbflUpdate(params: MdlJeddahFlnbflDtbase) {
    try {
        const rspnse = await ApiGlobalAxiospParams.post("/jeddah/flnbfl/update", params);
