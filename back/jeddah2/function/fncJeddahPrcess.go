...
// // TEMPORARY UPDATE CANCEL GET REAL TIME
...
// 		{"routfl": "-"}, {"routfl": ""}, {"routfl": bson.M{"$exists": false}},
// 	}})
// 	if err != nil {
// 		panic(err)
// 	}
...
// 	// Append to slice
// 	var mgomdl []mongo.WriteModel
// 	for datarw.Next(contxt) {
// 		var object mdl_jeddah.MdlJeddahSmrpnrDtbase
// 		datarw.Decode(&object)

// 		// Final push update
// 		timecr, _ := strconv.Atoi(object.Prmkey[6:16])
// 		object.Timecr = int64(timecr)
// 		object.Timedp = int64(0)
// 		object.Timerv = int64(0)
// 		onemdl := mongo.NewUpdateOneModel().
// 			SetFilter(bson.M{"prmkey": object.Prmkey}).
// 			SetUpdate(bson.M{"$set": object}).
// 			SetUpsert(true)
// 		mgomdl = append(mgomdl, onemdl)
// 	}

// 	// Push to database
// 	if len(mgomdl) > 0 {
// 		rspBlkwrt := fnc_global.FncGlobalDtbaseBlkwrt(mgomdl, "jeddah_pnrsmr")
// 		if !rspBlkwrt {
// 			fmt.Println("ERR LOG HERE, CAN'T INPUT jeddah_pnrsmr LAST")
// 		}
// 	}
// }

func FncJeddahPrcessMainpg(c *gin.Context) {
	if fnc_global.IsStat == "Done" {
		fnc_global.IsStat = "Wait"

		// Insdicaor Process Start
		totWokrer := 9
		nowTimenw := time.Now().AddDate(0, 0, -0).Format("0601021504")
		slcFlnbfl, sycFlnbfl := FncJeddahFlnbflSycmap(nowTimenw[0:6])
		prvDatefl := FncJeddahLogactLstdta()
		slcDrules := FncJeddahDrulesSycmap()
		sycAgtnme := FncJeddahAgtnmeSycmap()
		sycLcnpun := FncJeddahLcnpunSycmap(prvDatefl)
		sycDtlpnr := FncJeddahDtlpnrSycmap(prvDatefl)
		idcAgtnme, idcPnrcde, idcFlnbfl := &sync.Map{}, &sync.Map{}, &sync.Map{}
		// slcFlnbfl = map[string][]mdl_jeddah.MdlJeddahFlnbdbDtbase{
		// 	"JT": []mdl_jeddah.MdlJeddahFlnbdbDtbase{
		// 		mdl_jeddah.MdlJeddahFlnbdbDtbase{
		// 			Prmkey: "250910JT92UPG",
		// 			Airlfl: "JT",
		// 			Datefl: 250910,
		// 			Dateup: 250729,
		// 			Depart: "UPG",
		...
		// 		},
		// 	},
		// 	"IU": []mdl_jeddah.MdlJeddahFlnbdbDtbase{
		// 		mdl_jeddah.MdlJeddahFlnbdbDtbase{
		// 			Prmkey: "250922IU572UPG",
		// 			Airlfl: "IU",
		// 			Datefl: 250922,
		// 			Dateup: 250910,
		// 			Depart: "UPG",
		...
		for airlfl, slice1 := range slcFlnbfl {
			...
			if !slices.Contains([]string{"JT", "ID", "IW", "IU", "OD", "SL"}, airlfl) {
				continue
			}

			// Get 10 API sessions/tokens
			slcRspssn, err := fnc_global.FncGlobalSssionMultpl(airlfl, totWokrer)
			lgcRspssn := err != nil || slcRspssn[0] == "" || len(slcRspssn) < 1
			if lgcRspssn {
				fmt.Println(airlfl)
			...
			// Prepare job queue
			jobFlbase := make(chan mdl_jeddah.MdlJeddahFlnbdbDtbase, 1000)
				...
				if len(slcRspssn) >= i+1 {
					if slcRspssn[i] != "" {
						...
						go FncJeddahPrcessWorker(i, &slcRspssn[i], &swg, jobFlbase,
							sycAgtnme, sycLcnpun, sycDtlpnr, sycFlnbfl, idcAgtnme,
							idcPnrcde, idcFlnbfl, nowTimenw, slcDrules)
					}
				}
			}
			...
			fnc_global.FncGlobalClsssnMultpl(slcRspssn)
		}

		// Return to done status
		fnc_global.IsStat = "Done"

		// Update done again
		intDatenw, _ := strconv.Atoi(nowTimenw[0:6])
		intTimenw, _ := strconv.Atoi(nowTimenw)
		logAction := mdl_jeddah.MdlJeddahLogactDtbase{
			Dateup: int32(intDatenw), Timeup: int64(intTimenw),
			Statdt: "Done"}
		rsupdt := fnc_global.FncGlobalDtbaseBlkwrt([]mongo.WriteModel{
...
func FncJeddahPrcessWorker(nbr int, tkn *string, swg *sync.WaitGroup,
	jobFlbase <-chan mdl_jeddah.MdlJeddahFlnbdbDtbase,
	sycAgtnme, sycLcnpun, sycDtlpnr, sycFlnbfl, idcAgtnme, idcPnrcde, idcFlnbfl *sync.Map,
	nowTimenw string, slcDrules []mdl_jeddah.MdlJeddahDrulesDtbase) {
	var mgomdlAgtnme []mongo.WriteModel
	var mgomdlLcnpun []mongo.WriteModel
	var mgomdlSmrfln []mongo.WriteModel
	var mgomdlSmrpnr []mongo.WriteModel
	var mgomdlDtlpnr []mongo.WriteModel
	var mgomdlFlnbfl []mongo.WriteModel
...
outlop:
	for flbase := range jobFlbase {
		...
		prvLcnpun := map[string]mdl_jeddah.MdlJeddahLogpnrDtbase{}
		if tmpPrvdtl, ist := sycLcnpun.Load(dbsAirlfl + dbsFlnbfl + dbsDepart + strDatenw); ist {
			if getPrvdtl, ist := tmpPrvdtl.(map[string]mdl_jeddah.MdlJeddahLogpnrDtbase); ist {
				prvLcnpun = getPrvdtl
		...
		prvDtlpnr := map[string]mdl_jeddah.MdlJeddahDtlpnrDtbase{}
		if tmpPrvdtl, ist := sycDtlpnr.Load(dbsAirlfl + dbsFlnbfl + dbsDepart + strDatenw); ist {
			if getPrvdtl, ist := tmpPrvdtl.(map[string]mdl_jeddah.MdlJeddahDtlpnrDtbase); ist {
				prvDtlpnr = getPrvdtl
		...
		if dbsDatefl < int32(intTimepv) {
			...
			// Fetch LC and PUN data
			slcOutpun, slcOutllc := FncGlobalLcnpunApisbr(*tkn, dbsFlnbfl,
				dbsDepart, dbsRoutfl, dbsAirlfl, strDatenw, nowTimenw)
			...
			// Looping LC data
			fnlIsjedh := false
			tmpSmrpnr := []mongo.WriteModel{}
			tmpDtlpnr := []mongo.WriteModel{}
			tmpLcnpun := []mongo.WriteModel{}
			tmpFlnbfl := []mongo.WriteModel{}
			for _, lcnpun := range slcOutllc {

				// Delcare Pnr log data
				fnlDtlpnr := mdl_jeddah.MdlJeddahDtlpnrDtbase{
				...
				if prv, ist := prvDtlpnr[lcnpun.Pnrcde]; ist {
					...
					_, cekIsjedh := FncJeddahItrmrlGetapi("", tkn, &fnlDtlpnr, sycFlnbfl, idcPnrcde, idcFlnbfl,
						sycAgtnme, &tmpSmrpnr, &tmpFlnbfl)
				...
				if prv, ist := prvLcnpun[lcnpun.Pnrcde]; ist {
					...
					if nowRemove > 0 {
						...
						// Get remarks only
						FncJeddahItrmrlGetapi("", tkn, &fnlDtlpnr, sycFlnbfl, idcPnrcde, idcFlnbfl,
							sycAgtnme, &tmpSmrpnr, &tmpFlnbfl)
			...
			case len(slcOutpun) != 0 || len(slcOutllc) != 0:
				...
				if len(prvLcnpun) > 0 {
					for _, lcnpun := range prvLcnpun {
						...
						if nowDtlpnr, ist := prvDtlpnr[lcnpun.Pnrcde]; ist {
							...
							// Get remarks history split or cancel
							strToflnm, _ :=
								FncJeddahItrmrlGetapi("rmv", tkn, &nowDtlpnr, sycFlnbfl, idcPnrcde, idcFlnbfl,
									sycAgtnme, &mgomdlSmrpnr, &mgomdlFlnbfl)
							...
							if strToflnm != "" {
								...
								nowDtlpnr.Totpax = nowDtlpnr.Totpax
			...
			case len(prvDtlpnr) == 0:
				...
				// Lcnpun not isset get ldn data
				slcOutldn := FncGlobalLdnxxxApisbr(*tkn, dbsFlnbfl,
					dbsDepart, dbsRoutfl, dbsAirlfl, strDatenw, nowTimenw)
				...
				for _, lcnpun := range slcOutldn {
					...
					// Default summary PNR
					defDtlpnr := mdl_jeddah.MdlJeddahDtlpnrDtbase{
					...
					// Get remarks history split or cancel
					strToflnm, cekIsjedh :=
						FncJeddahItrmrlGetapi("rmv", tkn, &defDtlpnr, sycFlnbfl, idcPnrcde, idcFlnbfl,
							sycAgtnme, &tmpSmrpnr, &tmpFlnbfl)
			...
			case len(prvDtlpnr) > 0:
				...
				for _, dtlpnr := range prvDtlpnr {
					if dtlpnr.Agtidn == "" {
						...
						// Get remarks history split or cancel
						FncJeddahItrmrlGetapi("rmv", tkn, &dtlpnr, sycFlnbfl, idcPnrcde, idcFlnbfl,
							sycAgtnme, &mgomdlSmrpnr, &mgomdlFlnbfl)
					}
				}
			...
			nmodelSmrfln := mongo.NewUpdateOneModel().
				SetFilter(bson.M{"prmkey": flbase.Prmkey}).
				SetUpdate(bson.M{"$set": mdl_jeddah.MdlJeddahSmrflnDtbase{
		...
		if len(mgomdlLcnpun) > lmtdta && len(mgomdlLcnpun) != 0 {
			rspBlkwrt := fnc_global.FncGlobalDtbaseBlkwrt(mgomdlLcnpun, "jeddah_pnrlog")
			if !rspBlkwrt {
		...
		if len(mgomdlDtlpnr) > lmtdta && len(mgomdlDtlpnr) != 0 {
			rspBlkwrt := fnc_global.FncGlobalDtbaseBlkwrt(mgomdlDtlpnr, "jeddah_pnrdtl")
			if !rspBlkwrt {
		...
		if len(mgomdlFlnbfl) > lmtdta && len(mgomdlFlnbfl) != 0 {
			rspBlkwrt := fnc_global.FncGlobalDtbaseBlkwrt(mgomdlFlnbfl, "jeddah_flnbfl")
			if !rspBlkwrt {
		...
		if len(mgomdlSmrpnr) > lmtdta && len(mgomdlSmrpnr) != 0 {
			rspBlkwrt := fnc_global.FncGlobalDtbaseBlkwrt(mgomdlSmrpnr, "jeddah_pnrsmr")
			if !rspBlkwrt {
	...
	for key, mgo := range map[string][]mongo.WriteModel{
		...
		if len(mgo) > 0 {
			rspBlkwrt := fnc_global.FncGlobalDtbaseBlkwrt(mgo, key)
			if !rspBlkwrt {
...
func FncJeddahAgtgetParams(dtlpnr *mdl_jeddah.MdlJeddahDtlpnrDtbase, istjed bool,
	idcAgtnme *sync.Map, fnlAgtnme *map[string]mongo.WriteModel) {

	// Get agent name Lcnpun
	keyAgtnme := dtlpnr.Airlfl + dtlpnr.Agtnme
	nowAgtnme := *fnlAgtnme
	tmpAgtnme := mdl_jeddah.MdlJeddahAgtnmeDtbase{
...
func FncJeddahItrmrlGetapi(rmv string, tkn *string, fnlDtlpnr *mdl_jeddah.MdlJeddahDtlpnrDtbase,
	sycFlnbfl, idcPnrcde, idcFlnbfl, sycAgtnme *sync.Map, mgomdlSmrpnr, mgomdlFlnbfl *[]mongo.WriteModel,
) (string, bool) {
	...
	if fnlDtlpnr.Agtidn == "" {
		...
		if rawAgtnme, ist := sycAgtnme.Load(keyAgtnme); ist {
			if getAgtnme, ist := rawAgtnme.(mdl_jeddah.MdlJeddahAgtnmeDtbase); ist {
	...
	arrRmrkit, err := fnc_global.FncGlobalItrmrlApisbr(*tkn, fnlDtlpnr.Pnrcde)
	if err == nil {
		...
		objSmrpnr := mdl_jeddah.MdlJeddahSmrpnrDtbase{
		...
		if len(arrItinry) > 0 {
			...
			cpnFltype := "Outgoing"
			tmpFlnbfl := map[string]mdl_jeddah.MdlJeddahFlnbdbDtbase{}
			for _, itinry := range arrItinry {
				...
				tmpFlnbfl[cpnPrmkey] = mdl_jeddah.MdlJeddahFlnbdbDtbase{
					Prmkey: cpnPrmkey,
...
// func FncJeddahPushdbFlnbfl(mgomdlFlnbfl *[]mongo.WriteModel,
// 	fnlAgtnme *map[string]mongo.WriteModel, pnrcde string) {

// 	// Push mongo pnrlog
// 	delete(*fnlAgtnme, pnrcde)
// 	if len(*mgomdlFlnbfl) != 0 {
// 		rspBlkwrt := fnc_global.FncGlobalDtbaseBlkwrt(*mgomdlFlnbfl, "jeddah_flnbfl")
// 		if !rspBlkwrt {
// 			fmt.Println("ERR LOG HERE, CAN'T INPUT FLNbfl")
// 		}
